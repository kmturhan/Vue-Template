...
client.on('message', function (topic, message) {
    ...
    //topicName'lere göre case'ler oluşturdum ve her bir case için farklı görevleri var.
    switch(topicName){
        case 'home/' + token :                  //Cihaz ilk bağlandığında buraya girecek
        ...
        case 'home/lg_tv/attribute/' + token :
            ...
            console.log('TEST : ',testArray[Object.keys(jsonData.params)])
            ...
            if(testArray[Object.keys(jsonData.params)] ==  selectedPinKey && typeof selectedPinKey != 'undefined'){
                ...
                connection.query(mysqlQuery,[token],function(err,results,fields){
                    results.forEach(item => {
                        var json = {
                            method: 'rpcCommand',
                            params: { command: 'pl', tvId: item.tv_id, swc: 1, getCmd: '19' },
        ...
        case 'home/philips_tv/attribute/'+token:
            ...
            if(testArray[Object.keys(jsonData.params)] ==  selectedPinKey && typeof selectedPinKey != 'undefined'){
                ...
                console.log('ArrayIDVALUE :',arrayIDValue)
                var selectedPinValue = arrayIDValue[1];
                if(selectedPinValue == 1) { selectedPinValue = 0; } else { selectedPinValue = 1 }
                mysqlQuery = "UPDATE lcd_devices_status SET " + selectedPinKey + " = ?, last_update = ? WHERE token = ? AND tv_id = ?";
                connection.query(mysqlQuery,[selectedPinValue,dateTime,token,selectedTvID],function (error, results, fields) {
                    if (error) throw error;
                    console.log('success')
                ...
                connection.query(mysqlQuery,[token],function(err,results,fields){
                    ...
                    json = {
                        method: 'rpcCommand',
                        params: { command: 'pl', swc: 2, getCmd: '19' },
                        values: tvIDArray
                    }
        ...
        case 'home/philips_tv/attributesUp/'+token:
            ...
            case 'home/philips_tv/create/'+token:
                if(jsonData.method == 'setDevice'){
                    ...
                    jsonTvIdList.forEach((item,index) => {
                        console.log('jsonTVIDLIST : ',item)
                        connection.query(mysqlQuery,[token],(err,result,fields) => {
                            console.log('philips',result)
                            if(result.length == 0) {
                                mysqlQuery = "INSERT INTO lcd_devices_status(token,tv_id,brand,ip_address) VALUES (?,?,?,?)";
                                connection.query(mysqlQuery,[token,item,tvBrand,ipAddress],(err,rows)=>{
                                    console.log(err)
                                    console.log('TESTASDF: ',token,item,tvBrand,ipAddress)
                    ...
                    mysqlQuery = "UPDATE lcd_devices_status SET model = ?, last_update = ? WHERE tv_id = ? AND token = ?"
                    ...
                    jsonTvIdList.forEach(function(item,index) {    
                        ...
                        connection.query(mysqlQuery,[tvModels[index],dateTime,item,token],(err, result, fields) => {
                            console.log(err);
                            console.log(result);
                            console.log(fields);
...
router.post('/nameUpdate',function(req,res){
    ...
    if(data.isBrightnessAuto == false && data.blackScreenAuto == false) {
        ...
        connection.query(mysqlQuery, [data.name,data.isBrightnessAuto,data.blackScreenAuto,data.deviceActive, data.deviceId],(err,results,fields) => {
        ...
        connection.query(mysqlQuery, [data.name,data.sunriseValue,data.sunsetValue,data.sunriseTime,data.sunsetTime,data.blackScreenOpenTime,data.blackScreenCloseTime,data.blackScreenTimeOptions,data.sunTimeOptions,JSON.stringify(data.blackScreenWeekData),data.deviceActive,data.deviceId],(err,results,fields) => {
            console.log('Name Update OK!');
            console.log(err);
            console.log(mysqlQuery);
        })
        ...
        connection.query(mysqlQuery, [data.name,data.sunriseValue,data.sunsetValue,data.sunriseTime,data.sunsetTime,data.blackScreenAuto,data.sunTimeOptions,data.deviceActive,data.deviceId],(err,results,fields) => {
            console.log('Name Update OK!');
            console.log(err);
            console.log(mysqlQuery);
        })
        ...
        connection.query(mysqlQuery, [data.name,data.blackScreenOpenTime,data.blackScreenCloseTime,data.isBrightnessAuto,data.blackScreenTimeOptions,JSON.stringify(data.blackScreenWeekData),data.deviceActive,data.deviceId],(err,results,fields) => {
            console.log('Name Update OK!');
            console.log(err);
            console.log(mysqlQuery);
