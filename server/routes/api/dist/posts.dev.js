...
router.post('/nameUpdate', function (req, res) {
  ...
  if (data.isBrightnessAuto == false) {
    ...
    connection.query(mysqlQuery, [data.name, data.deviceId], function (err, results, fields) {
    ...
    connection.query(mysqlQuery, [data.name, data.sunriseValue, data.sunsetValue, data.sunriseTime, data.sunsetTime, data.blackScreenOpenTime, data.blackScreenCloseTime, data.deviceId], function (err, results, fields) {
      console.log('Name Update OK!');
      console.log(err);
