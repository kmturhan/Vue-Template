               ...
               <div class="d-custom-flex align-items-center justify-space-between">
                  ...
                  <div>
                     <span :class="item.color" class="fw-bold d-flex align-items-center">
                       ...
                       <span v-if="item.TvStatus == 0 && item.Connection_Status == 1" style="color: #e4002b !important;font-weight: bold;">CLOSE</span>
                     </span>
                  </div>   
               </div> 
            </div>
    ...
    loadData() {
      ...
      axios.get('http://192.168.10.46:5000/api/loadDevices').then(resp => {
        resp.data.forEach(item => {
          ...
          if (item.TvStatus == 0 || item.Connection_Status == 0) {
