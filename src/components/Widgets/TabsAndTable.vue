...
export default {
	...
	mqtt: {
		...
		'home/lg_tv/attribute/#' : function(val) {
			console.log('tesssst');
			var test = String.fromCharCode.apply(null,val);
			var jsonData = JSON.parse(test);
	...
	methods: {
		...
		*/		
		...
		this.deviceList.forEach(item => {
			if(item.token == token && item.tv_id == tvID) {
				if(item.tv_status == 0) {
					value = 2;
				}else {
					value = 1;
		...
		loadData() {
			...
			axios.get('http://192.168.10.31:5000/api/loadLcdDevices').then(resp => {
